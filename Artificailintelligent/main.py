...
y = iris['class'].copy()
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)#테스트셋 분류
scaler = StandardScaler()
scaler.fit(X_train, y_train)#스케일러 데이터 전처리
...
gnb_clf = GaussianNB()
gnb_clf.fit(X_train_scale, y_train)  # 테스트셋 학습 ->predict 대신 교차검증 사용
y_pred = cross_val_predict(gnb_clf, X, y, cv=3)  # 교차평가

print(classification_report(y, y_pred))  # 교차평가 한 리포트 출력
